...
def split_data_three_way(
    ...
    """
    Patient-level 70/10/20 split → (train_df, dev_df, test_df).

    The dev set is used for early stopping only. The test set is held out
    until final evaluation and must never influence model selection.
    Delegates to graph_constructor.split_three_way() as the single source
    of truth for the split logic (seed=42 fixed for reproducibility).

    Parameters
    ----------
    df : pd.DataFrame
        Full dataset with 'stay_id' column.
    train_ratio : float
        Fraction of patients in the training split (default 0.70).
    dev_ratio : float
        Fraction of patients in the development split (default 0.10).

    Returns
    -------
    train_df, dev_df, test_df : (pd.DataFrame, pd.DataFrame, pd.DataFrame)
    """
    import sys
    import os
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../../../src"))
    from graph_constructor import split_three_way
    train_ids, dev_ids, test_ids = split_three_way(df, train_ratio, dev_ratio, seed=42)
    return (
        df[df["stay_id"].isin(train_ids)],
        df[df["stay_id"].isin(dev_ids)],
        df[df["stay_id"].isin(test_ids)],
    )


def evaluate_model(
    targets: np.ndarray, predictions: np.ndarray, task_type: str
) -> Dict[str, float]:
    ...
    if task_type == "classification":
        ...
        return {"mse": mse, "rmse": np.sqrt(mse), "mae": np.mean(np.abs(targets - predictions))}


def get_baseline_metrics(
    train_targets: np.ndarray, val_targets: np.ndarray, task_type: str
...
def print_results(
    ...
    if task_type == "classification":
        ...
        print(f"Test Accuracy: {model_metrics['test']['accuracy']:.3f}")
        print(f"Test AUROC: {model_metrics['test']['auroc']:.3f}")
        print(f"Test AUPRC: {model_metrics['test']['auprc']:.3f}")
        ...
        print(f"Test Accuracy: {baseline_metrics['test']['accuracy']:.3f}")
        print(f"Test AUROC: {baseline_metrics['test']['auroc']:.3f}")
        print(f"Test AUPRC: {baseline_metrics['test']['auprc']:.3f}")
    else:  # regression
        print(f"Train RMSE: {model_metrics['train']['rmse']:.3f}")
        print(f"Train MAE: {model_metrics['train']['mae']:.3f}")
        print(f"Test RMSE: {model_metrics['test']['rmse']:.3f}")
        print(f"Test MAE: {model_metrics['test']['mae']:.3f}")
        ...
        print(f"Test RMSE: {baseline_metrics['test']['rmse']:.3f}")
        print(f"Test MAE: {baseline_metrics['test']['mae']:.3f}")
...
def run_benchmark(
    ...
    task_type = (
        "classification"
        if task in ["mechvent", "morta_hosp", "septic_shock", "sepsis", "vasopressor"]
        else "regression"
    )

    # Load and prepare data
    df = load_data(
        r"C:\Users\edmar\MeusProjetos\edmar-ppgc-dissertation\src\processed_files\patient_timeseries_v4.csv"
    )
    ...
    processor = TimeSeriesDataProcessor(
        ...
        if task in ["septic_shock", "mechvent", "sepsis", "vasopressor"]
        else None,
    )

    # Three-way patient-level split: train (70%) / dev (10%) / test (20%).
    # dev  → used internally by LSTM/Transformer for early stopping only.
    # test → held out; never influences training or model selection.
    train_df, dev_df, test_df = split_data_three_way(df)

    # Process and normalize data.
    # Scaler is fitted on train_features and reused for dev and test.
    print("\nProcessing data...")
    train_features, train_targets = processor.prepare_data(train_df)
    dev_features, dev_targets     = processor.prepare_data(dev_df)
    test_features, test_targets   = processor.prepare_data(test_df)

    train_features_norm, dev_features_norm = processor.normalize_features(
        train_features, dev_features
    )
    # normalize_features fits on train_features and transforms both args;
    # calling with test_features as the second arg reuses the same scaler.
    _, test_features_norm = processor.normalize_features(
    ...
    if model_type == "linear":
        ...
        if task_type == "regression":
            if regularization == "ridge":
                ...
                print(
                    f"Using ElasticNet regression with alpha={alpha}, l1_ratio=0.5 (combined L1/L2 regularization)"
                )
            else:
    ...
    if batch_size:
        ...
        test_preds  = model.predict(test_features_norm,  batch_size=batch_size)
    else:
    ...
    # Calculate metrics — report on held-out test set only.
    model_metrics = {
        "train": evaluate_model(train_targets, train_preds, task_type),
        "test":  evaluate_model(test_targets,  test_preds,  task_type),
    }

    baseline_metrics = get_baseline_metrics(train_targets, test_targets, task_type)
        ...
        for model_type in model_types:
            for include_treatments in treatment_options:
                if task_type == "static":
                    ...
                    print(
                        f"Task: {task}, Model: {model_type}, Include Treatments: {include_treatments}"
                    )
                    print(f"{'=' * 80}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--run_selected", action="store_true", help="Run all models for a specific task"
    )
